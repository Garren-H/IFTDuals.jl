...
IFTDuals.jl is a lightweight Julia package for computing higher order derivatives for a function, $x=g(\thetha)$ implicitly defined through the implicit function theorem (IFT) using dual numbers. IFT gived the relationship between $g$ and $\theta$ implicitly through the equation $f(g(\theta), \theta) = 0$. 
